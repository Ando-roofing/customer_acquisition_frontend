...
export default function MyVerification() {
  ...
  const fetchSubmissions = async () => {
    try {
      const res = await axios.get(
        "http://127.0.0.1:8000/verifications/visits/my-verifications/",
        {
  ...
  const columns = React.useMemo(
    () => [
      ...
      {
        Header: "Submitted By",
        accessor: (row) => row.submitted_by_name || row.submitted_by?.username,
      },
      {
        Header: "Status",
        accessor: "status",
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div className="d-flex justify-content-end gap-2">
            <Link
              to={`/verification-details/${row.original.id}`}
            ...
            >
              <i className="bi bi-eye"></i> View Detail
                 ...
                 <Link
                        to={`/verification-approve/${row.original.id}`}
                        className="btn btn-sm btn-outline-warning"
                        >
                        <i className="bi bi-pencil-square"></i> Verify
                        </Link>
                 <Link
                        to={`/verification-message/${row.original.id}`}
                        className="btn btn-sm btn-outline-warning"
                        >
